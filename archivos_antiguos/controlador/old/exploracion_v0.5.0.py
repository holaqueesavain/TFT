    ...
    try:
        while paso < MAX_PASOS:
            ...
            for dx, dy in direcciones:
                ...
                # 🔥 SI MEJORA RESPECTO A LA POSICIÓN AL INICIO DEL PASO, QUEDARSE
                if dist1 <= dist_actual - MEJORA_MINIMA:
                    ...
                    break  # salir del for: ya se movió
                else:
                    # Volver y probar siguiente
                    rtde_c.moveL([pose_actual[0], pose_actual[1], Z_PLANO] + rotacion_abajo, speed=0.15, acceleration=0.15)
                    time.sleep(0.1)
            ...
            if mejora_encontrada:
                ...
                # 🔥 NINGUNA DIRECCIÓN MEJORÓ ≥ 3 píxeles → SALTO EXPLORATORIO
                print("🌀 Sin mejora ≥ 3 px. Ejecutando salto exploratorio de 10 cm...")
                ...
                # Calcular nueva posición propuesta (10 cm = 0.10 m)
                x_salto = pose_actual[0] + dx_salto * 0.10
                y_salto = pose_actual[1] + dy_salto * 0.10

                # Verificar límites
                if not (LIMITE_X_MIN <= x_salto <= LIMITE_X_MAX):
                    x_salto = np.clip(x_salto, LIMITE_X_MIN, LIMITE_X_MAX)
                if not (LIMITE_Y_MIN <= y_salto <= LIMITE_Y_MAX):
                ...
                pose_actual = rtde_r.getActualTCPPose()
                HISTORIAL.append(pose_actual.copy())
                direccion_actual = None  # resetear dirección preferida
                fallas_consecutivas = 0  # resetear contador
